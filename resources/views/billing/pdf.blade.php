...
@if($invoice->verifiedPayments->isNotEmpty())<div class="payments"><strong>Payments</strong><table class="items"><thead><tr><th>Date</th><th>Method / Reference</th><th class="right">Amount</th></tr></thead><tbody>@foreach($invoice->verifiedPayments as $payment)<tr><td>{{ $payment->paid_at->format('F j, Y g:i A') }}</td><td>{{ $payment->method }}{{ $payment->reference ? ' · '.$payment->reference : '' }}</td><td class="right">PHP {{ number_format($payment->amount,2) }}</td></tr>@endforeach</tbody></table></div>@endif
